...
public class DivisibleOrDigitPresentProcessorTest {
    ...
    public void setup() {
        ...
        numberProcessor = factory.getProcessor(ProcessorTypeEnum.DIVISIBLE_OR_DIGIT_PRESENT_PROCESSOR);
    }

    @Test
    public void numberProcessorFactoryTest() {
        Assert.assertNotNull(numberProcessor);
        Assert.assertTrue(numberProcessor instanceof DivisibleOrDigitPresentProcessor);
    }
    ...
    @Test
    public void digitPresentNumberTest() {
        List<BaseNumber> numbers = new ArrayList<>();
        numbers.add(new MagicNumber3());
        numbers.add(new MagicNumber5());

        String text = numberProcessor.processNumber(23, numbers);
        Assert.assertEquals("Fizz", text);

        text = numberProcessor.processNumber(31, numbers);
        Assert.assertEquals("Fizz", text);

        text = numberProcessor.processNumber(55, numbers);
        Assert.assertEquals("Buzz", text);

        text = numberProcessor.processNumber(51, numbers);
        Assert.assertEquals("FizzBuzz", text);
    }
